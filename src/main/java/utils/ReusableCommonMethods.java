...
public class ReusableCommonMethods {
    ...
    public void safeClick(WebDriver driver, WebElement element, String name, int timeoutSec) {
        ...
        while (attempt < MAX_ATTEMPTS) {
            ...
            try {
                ...
                logger.warn("♻️ Element '{}' went stale on attempt {}. You may need to re-locate it before retrying.", name, Integer.valueOf(attempt));
            } catch (TimeoutException te) {
                logger.warn("⏳ Timeout waiting for '{}' to be visible/clickable on attempt {}.", name, Integer.valueOf(attempt));
            } catch (Exception e) {
    ...
    public void scrollIntoViewCenter(WebElement element) {
        try {
            ...
            logger.warn("⚠️ scrollIntoViewCenter failed: {}", e.toString());
        }
    }
    ...
    public long logLoadTimeAndReturnMs(String label, Instant start, long warnMs, long failMs) {
        ...
        if (elapsedMs >= failMs) {
            String msg = String.format("%s took %s s — more than %d s. Failing (SLA %ds).", label, secStr, failSec, failSec);
            logger.error(msg);
            logToAllure("❌ Load Time Failure", msg);
        } else if (elapsedMs >= warnMs) {
            String msg = String.format("%s took %s s — more than %d s.", label, secStr, warnSec);
            logger.warn(msg);
    ...
    public boolean clickWithRetry(WebElement element, String label) {
        ...
        for (int attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                logger.info("🔁 Attempt {} to click '{}'", attempt, label);
                ...
                logger.info("✔ Clicked '{}' via normal click.", label);
                return true;

            } catch (StaleElementReferenceException sere) {
                logger.warn("♻ '{}' stale — refreshing element and retrying...", label);
                // Allow retry — the caller must re-find element if needed
                sleep(200);

            } catch (ElementNotInteractableException intercept) {
                logger.warn("⚠ '{}' center overlapped; nudging scroll and retrying...", label);
                scrollNudge();
                sleep(200);

            } catch (Exception e) {
                logger.warn("⚠ Unexpected click failure on '{}': {} — retrying...", label, e.getMessage());
                scrollNudge();
                sleep(200);
            }
        }
        ...
        try {
            logger.warn("⚠ Normal clicking failed for '{}' — attempting JS click.", label);
            ((JavascriptExecutor) driver).executeScript("arguments[0].click();", element);
            logger.info("✔ JS Click succeeded for '{}'", label);
            return true;
        } catch (Exception e) {
            logger.error("❌ JS Click also failed for '{}': {}", label, e.getMessage());
            return false;
        }
    }
    ...
    public void safeType(WebElement element, String value, String fieldName, int timeoutSeconds) {
        try {
            ...
            logger.info("✅ Typed '{}' into '{}'", value, fieldName);
        } catch (Exception e) {
            logger.error("❌ Failed to type into '{}': {}", fieldName, e.getMessage(), e);
            throw e;
        }
    }
    ...
    public void typeText(WebElement element, String text, String fieldName) {
        try {
            ...
            logger.info("⌨ Entered '{}' into field '{}'", text, fieldName);

            AllureLoggerUtils.logToAllure("Text Input",
                    "Typed: <b>" + text + "</b> into field: <b>" + fieldName + "</b>");

        } catch (Exception e) {
            logger.error("❌ Failed to type text '{}' into '{}': {}", text, fieldName, e.getMessage());
